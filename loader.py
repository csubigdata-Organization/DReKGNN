...
def data_reader(logger, config, dataset="Fdataset", disease_disease_topk=7, drug_drug_topk=7):
    ...
    feat_path = "./feat/" + dataset + "/"
    llm_model = 'llama'
    dr_feat = torch.load(os.path.join(feat_path, 'drug'+ '_' + llm_model + '_embedding.pt'))
    di_feat = torch.load(os.path.join(feat_path, 'disease'+ '_' + llm_model + '_embedding.pt'))
    ...
    return disease_disease_sim_Matrix, drug_drug_sim_Matrix, truth_label, train_data, test_data, dr_feat, di_feat
...
def case_data_reader(logger, config, dataset="Fdataset", disease_disease_topk=7, drug_drug_topk=7):
    ...
    return disease_disease_sim_Matrix, drug_drug_sim_Matrix, train_matrix, train_data, test_data, dr_feat, di_feat
...
def analysis_data_reader(config, logger, specific_id, dataset="Fdataset", disease_disease_topk=7, drug_drug_topk=7):
    ...
    return disease_disease_sim_Matrix, drug_drug_sim_Matrix, train_matrix.T, train_data, test_data, dr_feat, di_feat
