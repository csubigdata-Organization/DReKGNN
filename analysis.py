...
def results_writer(specific_name, config):
    path = "./data/" + config['dataset'] + "/"
    if config['dataset'] in ['Fdataset', 'Cdataset']:
        _, _, _, _, disease_name, _, _ = load_mat(path + config['dataset'] + ".mat")
    elif config['dataset'] == "LRSSL":
        _, _, _, _, disease_name, _, _ = load_lrssl(path)

    # path = "./data/" + config['dataset'] + "/"
    # if config['dataset'] in ['Fdataset', 'Cdataset']:
    #     _, _, drug_name, _, _, _, _ = load_mat(path + config['dataset'] + ".mat")
    # elif config['dataset'] == "LRSSL":
    #     _, _, drug_name, _, _, _, _ = load_lrssl(path)
    # _, _, _, _, disease_name, _, _ = load_mat(path + ".mat")
    score = scio.loadmat("result/analysis/{}/{}.mat".format(config['dataset'], specific_name))
    y_score = score["predict"].reshape(-1)
    for i in range(len(disease_name)):
        if config['dataset'] in ['Fdataset', 'Cdataset']:
            disease_name[i] = disease_name[i][0]
        elif config['dataset'] == "LRSSL":
            disease_name[i] = disease_name[i]
    ...
    path = os.path.join(".", "result/analysis/{}/".format(config['dataset']))
...
def analysis_results(config):
    ...
    # mat = scio.loadmat("./data/{}/{}.mat".format(config['dataset'], config['dataset']))
    # drug_sim = mat["drug"].astype(np.float)
    # drug_name = mat["Wrname"].reshape(-1)
    path = "./data/" + config['dataset'] + "/"
    if config['dataset'] in ['Fdataset', 'Cdataset']:
        drug_sim, _, drug_name, _, _, _, _ = load_mat(path + config['dataset'] + ".mat")
    elif config['dataset'] == "LRSSL":
        drug_sim, _, drug_name, _, _, _, _ = load_lrssl(path)
    ...
    for i in range(len(top_triplets)):
        if config['dataset'] in ['Fdataset', 'Cdataset']:
            ...
            drug_set.add(top_triplets[i][0])
            drug_set.add(top_triplets[i][1])
    ...
    final_all_id_auroc, final_all_id_aupr = [], []
    for id in drug_id_list:
        if config['dataset'] in ['Fdataset', 'Cdataset']:
            config["specific_name"] = drug_name[id][0]
        elif config['dataset'] == "LRSSL":
            config["specific_name"] = drug_name[id]
        disease_disease_sim_Matrix, drug_drug_sim_Matrix, train_matrix, all_train_mask, \
        all_test_mask, dr_feat, di_feat = analysis_data_reader(config=config, logger=logger, specific_id=id, dataset=config['dataset'],
        ...
        for fold_num in range(len(all_train_mask)):
            ...
            with tf.Session(config=tf_config) as sess:
                ...
                model = create_model(sess, Model, ckptpath, config, logger, dr_feat, di_feat)
        ...
        final_all_id_auroc.append(final_test_auroc),
        final_all_id_aupr.append(final_test_aupr)

        results_writer(config['specific_name'], config)
    print("final_avg_auroc :{:>.5f} final_avg_aupr :{:>.5f}".format(np.mean(final_all_id_auroc),
                                                                    np.mean(final_all_id_aupr)))
...
def evaluate(sess, model, name, data, logger, all_scores, all_labels, config, fold_num=0):
    ...
    if name == "valid":
        ...
        folder = os.path.join('result', "analysis", config['dataset'])
