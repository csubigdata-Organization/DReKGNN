...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='Fdataset', help='Choose a dataset. [Fdataset/Cdataset]')
    parser.add_argument('--use_LLM', type=int, default=1)  # 0 is False, 1 is true
    ...
    parser.add_argument('--mode', type=str, default='cv', help='cv, analysis.')
...
def cross_validation():
    ...
    if os.path.isfile(map_file_path):
        ...
        disease_disease_sim_Matrix, drug_drug_sim_Matrix, truth_label, \
        all_train_mask, all_test_mask, dr_feat, di_feat = data_reader(logger,
                                                    config=config,
                                                    dataset=dataset,
    ...
    for fold_num in range(len(all_train_mask)):
        ...
        with tf.Session(config=tf_config) as sess:
            ...
            # save _model(sess, model, ckptpath, logger)
    ...
    print('\t\t\t final_avg_auroc:{:>.5f}, final_avg_auroc_std:{:>.3f}'.format(np.mean(final_all_auroc), np.std(final_all_auroc)))
    print('\t\t\t final_avg_aupr:{:>.5f}, final_avg_aupr_std:{:>.3f}'.format(np.mean(final_all_aupr), np.std(final_all_aupr)))
